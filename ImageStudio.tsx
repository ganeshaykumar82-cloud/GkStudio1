...
const ImageStudio = ({ initialProject }: { initialProject?: Project | null }) => {
  ...
  // Extend tool type for eyedropper
  ...
  const startDrawing = useCallback((e: any) => {
    ...
    if (activeTab === 'draw' && drawSettings.tool === 'eyedropper') {
        const p = ctx.getImageData(x, y, 1, 1).data;
        const hex = rgbToHex(p[0], p[1], p[2]);
        setDrawSettings(d => ({...d, color: hex, tool: 'brush'})); // Switch back to brush after picking
        return;
    }
  ...
  const drawMove = useCallback((e: any) => {
    if ((!isDrawingRef.current && !isSelecting) || !imgSrc || drawSettings.tool === 'text' || drawSettings.tool === 'eyedropper') return;
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
    ...
    <div className="flex h-full bg-[#1a1a1a] relative">
      <Sidebar>
         ...
         <div className="mt-4 p-4 border-t border-neutral-800 flex-1 overflow-y-auto">
             ...
             {activeTab === 'draw' && (
                 <div className="space-y-6">
                     <div className="grid grid-cols-4 gap-2 p-1 bg-neutral-900 rounded border border-neutral-800">
                         ...
                         <button onClick={() => setDrawSettings(d => ({...d, tool: 'eyedropper'}))} className={`p-2 rounded flex flex-col items-center justify-center ${drawSettings.tool === 'eyedropper' ? 'bg-blue-600 text-white' : 'text-neutral-400'}`}>
                             <Pipette className="w-4 h-4 mb-1"/> <span className="text-[8px] uppercase font-bold">Picker</span>
                         </button>
                     </div>
