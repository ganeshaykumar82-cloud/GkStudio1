...
const VideoStudio = ({ initialProject }: { initialProject?: Project | null }) => {
  ...
  const moveLayer = (index: number, direction: 'up' | 'down') => {
      ...
      if (direction === 'up' && index > 0) {
          // In UI list, up means lower index (0 is top of list).
          // But for rendering (z-index), usually last is on top.
          // Let's assume list order = z-order (last is top) for rendering in `layers.forEach`.
          // So 'Moving Up' visually in the list usually means increasing priority.
          // Let's swap with next element to move "up" the stack (towards end of array).
          if (index < newLayers.length - 1) {
              [newLayers[index], newLayers[index + 1]] = [newLayers[index + 1], newLayers[index]];
              updateLayers(newLayers);
          }
      } else if (direction === 'down' && index > 0) {
          // Move "down" the stack (towards start of array)
          [newLayers[index], newLayers[index - 1]] = [newLayers[index - 1], newLayers[index]];
          updateLayers(newLayers);
      }
  };
