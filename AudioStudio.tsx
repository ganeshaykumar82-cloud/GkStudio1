...
import { Mic, Sliders, Sparkles, Play, Pause, Settings2, Volume2, Square, Download, ArrowRight, Save, Music, Zap, Gauge, Waves, X, Settings, Undo, Redo, ListMusic, Disc, Music4, Drum, Scissors, Copy, Clipboard, Trash, ZoomIn, ZoomOut, Crop, MousePointer2, Flag, Circle, StopCircle, Speaker, Radio, Maximize2, Move, ChevronRight, TrendingUp, TrendingDown, Repeat, Grid3X3, Users, FolderOpen, FileJson, Trash2 } from 'lucide-react';
...
const AudioStudio = ({ initialProject }: { initialProject?: Project | null }) => {
  ...
  const handleClearTrack = () => {
      if (confirm("Are you sure you want to clear the current track? Unsaved changes will be lost.")) {
          setAudioSrc(null);
          setAudioBuffer(null);
  ...
  const handleExport = async () => {
      if (!audioBuffer) {
          alert("No audio track loaded to export. Please import a file first.");
          return;
      }
      setIsExporting(true);
                 ...
                 <div className="space-y-6">
                    <div className="bg-neutral-900 p-4 rounded-xl border border-neutral-800 shadow-inner relative overflow-hidden group">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xs font-bold text-green-400 uppercase tracking-widest flex items-center">
                                <Disc className="w-4 h-4 mr-2 animate-[spin_3s_linear_infinite] opacity-0 group-hover:opacity-100 transition-opacity"/> 
                                Master Deck
                            </h3>
                            {audioSrc && (
                                <button onClick={handleClearTrack} className="text-neutral-500 hover:text-red-500 p-1" title="Eject Track">
